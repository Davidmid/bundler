...
RSpec.describe "bundle add" do
  ...
  it "shows error message when version is not formatted correctly" do
    ...
    expect(compat_err).to match("Invalid gem requirement pattern '~>1 . 0'")
  end

  it "shows error message when gem cannot be found" do
    bundle "add 'werk_it'"
    expect(compat_err).to match("Could not find gem 'werk_it' in")

    bundle "add 'werk_it' -s='file://#{gem_repo2}'"
    expect(compat_err).to match("Could not find gem 'werk_it' in rubygems repository")
  end

  it "shows error message when source cannot be reached" do
    bundle "add 'baz' --source='http://badhostasdf'"
    expect(compat_err).to include("Could not reach host badhostasdf. Check your network connection and try again.")

    bundle "add 'baz' --source='file://does/not/exist'"
    expect(compat_err).to include("Could not fetch specs from file://does/not/exist/")
  end
  ...
  describe "with --optimistic and --strict" do
    it "throws error" do
      ...
      expect(compat_err).to include("You can not specify `--strict` and `--optimistic` at the same time")
    end
  end
  ...
  context "multiple gems" do
    ...
    it "throws error if any of the specified gems are present in the gemfile with different version" do
      ...
      expect(compat_err).to include("You cannot specify the same gem twice with different version requirements")
      expect(compat_err).to include("You specified: weakling (~> 0.0.1) and weakling (>= 0).")
    end
  end
  ...
  describe "when a gem is added which is already specified in Gemfile with version" do
    it "shows an error when added with different version requirement" do
      ...
      expect(compat_err).to include("You cannot specify the same gem twice with different version requirements")
      expect(compat_err).to include("If you want to update the gem version, run `bundle update rack`. You may also need to change the version requirement specified in the Gemfile if it's too restrictive")
    end
    ...
    it "shows error when added without version requirements" do
      ...
      expect(compat_err).to include("Gem already added.")
      expect(compat_err).to include("You cannot specify the same gem twice with different version requirements")
      expect(compat_err).not_to \
        include("If you want to update the gem version, run `bundle update rack`. You may also need to change the version requirement specified in the Gemfile if it's too restrictive")
    end
  end
  ...
  describe "when a gem is added which is already specified in Gemfile without version" do
    it "shows an error when added with different version requirement" do
      ...
      expect(compat_err).to include("You cannot specify the same gem twice with different version requirements")
      expect(compat_err).to include("If you want to update the gem version, run `bundle update rack`.")
      expect(compat_err).not_to include("You may also need to change the version requirement specified in the Gemfile if it's too restrictive")
    end
  end
end
