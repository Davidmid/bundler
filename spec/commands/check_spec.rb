...
RSpec.describe "bundle check" do
  ...
  it "prints a generic error if the missing gems are unresolvable" do
    ...
    bundle :check
    expect(compat_err).to include("Bundler can't satisfy your Gemfile's dependencies.")
  end
  ...
  it "ensures that gems are actually installed and not just cached" do
    ...
    bundle "check"
    expect(compat_err).to include("* rack (1.0.0)")
  ...
  it "outputs an error when the default Gemfile is not found" do
    ...
    expect(compat_err).to include("Could not locate Gemfile")
  end
  ...
  context "--path vendor/bundle after installing gems in the default directory" do
    it "returns false" do
      ...
      bundle "check --path vendor/bundle"
      expect(exitstatus).to eq(1) if exitstatus
      expect(compat_err).to match(/The following gems are missing/)
    end
  end
  ...
  describe "when locked" do
    ...
    it "shows what is missing with the current Gemfile if it is not satisfied" do
      ...
      bundle :check
      expect(compat_err).to match(/The following gems are missing/)
      expect(compat_err).to include("* rack (1.0")
    end
  end
